  ...
  const enviarMensaje = async () => {
    ...
    try {
      ...
      const prompt = `
        ...
        - Dim_Cliente (id_cliente, nombre, apellido, tipo_cli, cedula)
        - Dim_Empleado (id_empleado, nombre, apellido, cedula)
        - Dim_Equipo_Computarizado (id_equipocomp, tipo, marca, color, modelo)
        - Dim_Servicio (id_ser, descripcion, costo)
        - Hecho_Diagnostico (id_diag, id_detallediag, id_equipocomp, id_cliente, id_empleado, id_ser, fecha, costo, total)
        Relaciones:
        - Hecho_Diagnostico.id_cliente -> Dim_Cliente.id_cliente
        - Hecho_Diagnostico.id_empleado -> Dim_Empleado.id_empleado
        - Hecho_Diagnostico.id_ser -> Dim_Servicio.id_ser
        - Hecho_Diagnostico.id_equipocomp -> Dim_Equipo_Computarizado.id_equipocomp
        - Hecho_Diagnostico.fecha -> Dim_Tiempo.fecha
        Instrucciones:
      ...
      const response = await fetch('http://127.0.0.1:3000/ia/consultarconia', {
        method: 'POST',
    ...
    <Modal show={mostrarChatModal} onHide={() => setMostrarChatModal(false)} size="lg">
      ...
      <Modal.Body>
        ...
        <Form.Control
          className="mt-3"
          type="text"
          placeholder="Escribe una consulta (ej. 'Diagnósticos totales por empleados en 2025')"
          value={mensaje}
